...
hfiles4=log.h rsm.h rsm_protocol.h config.h paxos.h paxos_protocol.h rsm_state_transfer.h rsmtest_client.h tprintf.h socketConnection.h
hfiles5=rsm_state_transfer.h rsm_client.h
rsm_files = rsm.cc paxos.cc config.cc log.cc handle.cc socketConnection.cc
...
ifeq ($(LAB6GE),1)
  lock_server+= $(rsm_files) socketConnection.cc
endif
ifeq ($(LAB7GE),1)
...
clean_files=rpc/rpctest rpc/*.o rpc/*.d rpc/librpc.a *.o *.d yfs_client extent_server lock_server lock_tester lock_demo rpctest test-lab-3-b test-lab-3-c rsm_tester socketConn
.PHONY: clean handin
clean: 
	rm $(clean_files) -rf 
